...
// Mirrors matrix.ts's busy-note helper (matrix paths bypass mcp.ts's page() wrapper).
...
export interface VerifyOpts { port?: number; states?: PseudoStates; name?: string; dir?: string }
...
//
// Contract 5: states apply to the check only. Diffing a forced-state layout against a
// resting snapshot would always show a change, so the diff always compares the RESTING
// layout. When both states and name are given that needs a second, unforced page load per
// viewport (worst case 2 loads/viewport) — when no states are forced, the check's own tree
// already IS the resting layout, so one load covers both.
